...
def _split(y, tx, col_index, col_value):
    """
    ...
    if isinstance(col_value, int):
        ...
        if len(y) != 0:
            ...
            value_y = non_value_y = []
        return [[value_y, np.delete(value_tx, col_index, axis=1), value_rows],
                [non_value_y, non_value_tx, non_value_rows]]
...
def split_jets_and_mass(y, tx):
    ...
    for group in groups:
        ...
        subgroups = _split(jet_y, jet_x, 0, -999)  # split based on mass for the first column
